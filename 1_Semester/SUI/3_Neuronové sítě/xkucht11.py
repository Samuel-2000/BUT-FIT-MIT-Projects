...
def tangent_minimum(function, x0, nb_steps):
    ...
    for _ in range(nb_steps):
        x0.data -= torch.autograd.grad(function(x0), x0)[0]
...
class MSE_with_regression:
    ...
    def __call__(self, y, t, model):
        mse = self.loss(y, t)
        l2 = 0.5 * self.l2_beta * model.l2_norm()**2
        return mse + l2
...
class MeteoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        x = torch.from_numpy(self.xs[idx]).float().squeeze()
        y = torch.from_numpy(np.array([self.ts[idx]])).float().squeeze()
        return x, y
...
def train(model, train_stream, optimizer, nb_updates, eval_period, valid_loader):
    ...
    model.train()
    for i in range(1, nb_updates + 1):
        # Získání minibatche z cyklického iterátoru
        X_batch, t_batch = next(train_stream)

        # Výpočet predikce a loss
        y_batch = model(X_batch)
        loss = torch.nn.functional.mse_loss(y_batch, t_batch).squeeze()

        # Zpětný průchod a aktualizace vah
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        # Vyhodnocení na validačních datech v pravidelných intervalech
        if i % eval_period == 0:
            valid_loss = evaluate(model, valid_loader)
            valid_progress.append((i, valid_loss.item()))
