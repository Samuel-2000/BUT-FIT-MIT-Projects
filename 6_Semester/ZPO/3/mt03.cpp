...
int passFilter( const cv::Mat& src, cv::Mat& dst, int limit_frequency, int flag, cv::Mat * spektrum = NULL )
{
	...
	int left   = (spectrumCplx.cols - limit_frequency) / 2;
	int right  = left + limit_frequency;
	int low    = (spectrumCplx.rows - limit_frequency) / 2;
	int high   = low + limit_frequency;

	if (flag == HIGH_PASS_FILTER) {
		// Zero out the central square (keep only high frequencies)
		for (int i = low; i < high; ++i)
			for (int j = left; j < right; ++j)
				spectrumCplx.at<cv::Vec2f>(i, j) = cv::Vec2f(0, 0);
	}
	else { // LOW_PASS_FILTER
		// Zero out everything outside the central square (keep only low frequencies)
		for (int i = 0; i < spectrumCplx.rows; ++i)
			for (int j = 0; j < spectrumCplx.cols; ++j)
				if (i < low || i >= high || j < left || j >= right)
					spectrumCplx.at<cv::Vec2f>(i, j) = cv::Vec2f(0, 0);
	}
	...
	cv::normalize( srcPadded, srcPadded, 0, 1, NORM_MINMAX );
...
int main(int argc, char* argv[])
{
	...
	cv::Mat src_gray;
	cv::cvtColor( src_rgb, src_gray, COLOR_BGR2GRAY );
...
cv::Mat spectrumMagnitude( cv::Mat & specCplx )
{
	...
	cv::normalize( specMag, specMag, 0, 1, NORM_MINMAX );
	return specMag;
}
