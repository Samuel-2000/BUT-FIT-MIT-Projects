	...
	*/
	...
	for (int y = 0; y < dst.rows; ++y) {
		for (int x = 0; x < dst.cols; ++x) {
			// Map destination pixel (x,y) to source coordinates using inverse transformation T
			float x_src = T.at<float>(0,0) * x + T.at<float>(0,1) * y + T.at<float>(0,2);
			float y_src = T.at<float>(1,0) * x + T.at<float>(1,1) * y + T.at<float>(1,2);
			int ix = cvRound(x_src);
			int iy = cvRound(y_src);
			if (ix >= 0 && ix < src.cols && iy >= 0 && iy < src.rows) {
				dst.at<uchar>(y, x) = src.at<uchar>(iy, ix);
			}
		}
	}
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	float rad = r * CV_PI / 180.0f;

	// Translation matrix (move center to origin)
	T.at<float>(0,0) = 1; T.at<float>(0,1) = 0; T.at<float>(0,2) = -c.x;
	T.at<float>(1,0) = 0; T.at<float>(1,1) = 1; T.at<float>(1,2) = -c.y;
	T.at<float>(2,0) = 0; T.at<float>(2,1) = 0; T.at<float>(2,2) = 1;

	// Rotation matrix (clockwise – this will produce a counter‑clockwise output after inversion)
	R.at<float>(0,0) = cos(rad); R.at<float>(0,1) = sin(rad); R.at<float>(0,2) = 0;
	R.at<float>(1,0) = -sin(rad); R.at<float>(1,1) = cos(rad); R.at<float>(1,2) = 0;
	R.at<float>(2,0) = 0; R.at<float>(2,1) = 0; R.at<float>(2,2) = 1;

	// Scaling matrix
	S.at<float>(0,0) = s; S.at<float>(0,1) = 0; S.at<float>(0,2) = 0;
	S.at<float>(1,0) = 0; S.at<float>(1,1) = s; S.at<float>(1,2) = 0;
	S.at<float>(2,0) = 0; S.at<float>(2,1) = 0; S.at<float>(2,2) = 1;

	// Compose forward transformation: translate to origin, rotate, scale, translate back
	M = (T.inv()) * (S * (R * T));
