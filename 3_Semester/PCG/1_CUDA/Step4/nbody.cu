 ...
 * @param N    - Number of particles
 */
__global__ void centerOfMass(const Particles p, float4* dCenterOfMass, int* dLock, unsigned int N) {
    extern __shared__ float4 sharedData[];

    unsigned int tid = threadIdx.x;
    unsigned int lane = tid % warpSize;
    unsigned int warpId = tid / warpSize;

    float posX = 0.0f, posY = 0.0f, posZ = 0.0f, weight = 0.0f;
    for (unsigned int i = blockIdx.x * blockDim.x + tid; i < N; i += gridDim.x * blockDim.x) {
        ...
        for (int offset = warpSize / 2; offset > 0; offset /= 2) {
            float tmpPosX = __shfl_down_sync(0xFFFFFFFF, posX, offset);
            float tmpPosY = __shfl_down_sync(0xFFFFFFFF, posY, offset);
            float tmpPosZ = __shfl_down_sync(0xFFFFFFFF, posZ, offset);
            float tmpWeight = __shfl_down_sync(0xFFFFFFFF, weight, offset);

            float weight_ratio = tmpWeight / (weight + tmpWeight);
            posX += (tmpPosX - posX) * weight_ratio;
            posY += (tmpPosY - posY) * weight_ratio;
            posZ += (tmpPosZ - posZ) * weight_ratio;
            weight += tmpWeight;
        }
        __syncthreads();
        if (lane == 0) {
            sharedData[warpId] = make_float4(posX, posY, posZ, weight);
            printf("%s", "");  // without print, the data becomes corrupted, and i couldnt solve it differently in 2 days. maybe shared memory doesnt like storing data to neighbouring banks from different warps at the same time.
        }
        __syncthreads();

        // Perform block-level reduction on warp results
        if (warpId == 0) {
            posX = (tid < blockDim.x / warpSize) ? sharedData[tid].x : 0.0f;
            posY = (tid < blockDim.x / warpSize) ? sharedData[tid].y : 0.0f;
            posZ = (tid < blockDim.x / warpSize) ? sharedData[tid].z : 0.0f;
            weight = (tid < blockDim.x / warpSize) ? sharedData[tid].w : 0.0f;

            // Repeat warp-level reduction on the block's warp-level results
            ...
            if (tid == 0) {
                while (atomicCAS(&dLock[0], 0, 1) != 0) {}

                float weight_ratio = dCenterOfMass->w > 0.0f ? weight / (dCenterOfMass->w + weight) : 1.0f;

                dCenterOfMass->x += (posX - dCenterOfMass->x) * weight_ratio;
                dCenterOfMass->y += (posY - dCenterOfMass->y) * weight_ratio;
                dCenterOfMass->z += (posZ - dCenterOfMass->z) * weight_ratio;
                dCenterOfMass->w += weight;

                atomicExch(&dLock[0], 0);
            }
        }
    }
}
